...
def register(request: HttpRequest) -> HttpResponse:
    if request.method == 'GET':
        return TemplateResponse(request, 'registration/register.html', {'form': UserCreationForm()})
...
def check_email(request: HttpRequest) -> HttpResponse:
    if (email := request.GET.get('email')) and User.objects.filter(email__iexact=email).exists():
        return HttpResponse(format_html("""<ul class="error-messages"><li>This email is in use.</li></ul>"""))
    return HttpResponse()

...
def favorite(request: HttpRequest, article_id: int) -> HttpResponse:
    ...
    return TemplateResponse(
        ...
        'articles/_favorite_action.html',
        {
            ...
            'is_action': True,
            'is_detail': False if request.htmx.target == f'favorite-{article.id}' else True,
        },
    )
...
def tags_autocomplete(request: HttpRequest) -> HttpResponse:
    ...
    tags = Tag.objects.filter(name__istartswith=search).distinct() if search else Tag.objects.none()
